...
def upload_file():
    ...
    if file:
        ...
        # Compare the uploaded ID proof with the captured face
        captured_face_path = "uploads/face_image.jpg"  # Path where the captured face is saved
        match = compare_faces(file_path, captured_face_path)

        if isinstance(match, str):
            return match  # Return error message if face detection failed
        
        if match:
            return 'ID proof is valid: Face matches!'
        else:
            return 'ID proof is invalid: Face does not match!'

    return 'ID proof uploaded successfully! You can now capture your face.'
